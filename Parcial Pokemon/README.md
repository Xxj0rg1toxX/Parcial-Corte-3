# Aclaracion
Hay 2 archivos de pokemon, v1 y v2, la v1 es la version inicial y completamente funcional, con 2 pokemon, 3 ataques, efectos de estado y combate por turnos mientras que la v2 es la version mas compleja y mas grande pero con algunos problemas en la implementacion de los efectos de estado y la separacion por headers y el combate por turnos esta dividido en etapas diferentes, en las que se elige un ataque y se ejecuta de una vez, el sistema de cambio de estadisticas puede estar mal implementado ya que algunas veces no se efectua sin embargo la logica esta presente para en algun momento porder escalarlo y mejorarlo. Queda a su criterio la calificacion de esta parte del proyecto ya que en la v2 quisimos ir mas alla de lo solicitado pero por falta de conocimiento se nos hizo complicado implementar estas funcionalidades.
